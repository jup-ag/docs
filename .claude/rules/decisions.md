...
**Decision:** Port as a public "DEX Integration" group in the Lend nav (last content group, directly above Resources, per YY), 4 pages: `index` (overview + condensed architecture), `typescript`, `cpi`, `errors`. Branded "Jupiter Lend DEX" (upstream says "Fluid DEX"). No links to the private source repo; pages are self-contained.
