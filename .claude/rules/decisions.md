...
| 371 dead paths (see PR #958) | per family, see below | 2026-09-24 | Dead-URL cleanup (BUILD-894): the Mintlify analytics inventory listed 1,875 paths returning 404 with 46,951 views in 90 days. Rules cover 97% of that traffic. Families: (1) old Docusaurus dev.jup.ag paths (`/swap-api/*` -> `/swap/v1/*` exact equivalents then `/swap`, `/ultra-api/*` -> `/ultra/*`, `/price-api*` -> `/price`, `/token-api*` -> `/tokens`, `/trigger-api/*`, `/recurring-api/:slug*` -> `/recurring/:slug*`, `/perp-api/*` -> `/perps/*`, `/lend-api/*`, `/send-api/:slug*`, `/studio-api/:slug*`, `/api-setup`/`/api-rate-limit`/`/api-payment-method`/`/api-faq` -> `/portal/*`, `/api/*` -> `/api-reference/*`, `/apis/*`, `/old/*`, `/legacy/*`, `/additional-topics/*`); (2) old station.jup.ag user guides (`/guides/general/*`, `/guides/swap/*`, `/guides/perpetual-exchange/*`, `/guides/jlp/*`, `/guides/limit-order/*`, `/guides/dca/*`, `/guides/onboard/*`, `/guides/jupsol/*`, `/guides/apepro/*`, `/guides/jupiter-lock/*`) -> the docs.jup.ag equivalent (`https://docs.jup.ag/user-docs/...`, external destinations); (3) paths guessed by AI agents on the current tree (`/swap/get-order`, `/swap/v2/order`, `/ultra/shield`, `/perps/fees`...) -> the page they meant. Plus `/docs/:slug*` -> `/:slug*` (double prefix, `/docs/send` alone was 2,425 views) and `/changelog/:slug*` -> the dev-platform changelog. Explicit rules precede their wildcard. ⚠️ `/ultra-api/*` -> `/ultra/*` must be repointed when BUILD-811 redirects `ultra/*` to Swap v2, or `check-redirects.js` will flag chains |
