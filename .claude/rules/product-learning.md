...
- [2026-07-09] Verified live (external key) against the beam api-v2 design spec (TX-105): `skipPreflight` must be `true` (explicit `false` → `-1015 "skipPreflight must be true"`), `maxRetries` must be `0` (non-zero → `-1015 "maxRetries must be 0"`), both default correctly if omitted. `swqosOnly` is an accepted optional config field (`true` = SWQoS-only, lands; recommended for latency, no Jito tip needed). External error codes: `-1013` insufficient tip, `-1014` missing tip, `-1015` invalid param, `-1016` bad encoding. (This corrects my earlier read that `skipPreflight` was only a perf rec: web3.js was omitting it, not sending `false`; explicit `false` is in fact rejected.)
