...
- [2026-07-22] Upstream staleness flag for the MCP team (successor to the resolved Trigger-V1 flag): the MCP's Recurring (DCA) domain still calls `/recurring/v1/*`, which is now the unmaintained DCA V1; current DCA is DCAv2 under `/trigger/v2` (docs: `trigger/dca.mdx`). The `recurring_build_order` minimums the MCP schema enforces (2 cycles, 50 USD/cycle) are V1 rules and are moot for docs (DEV-744 canceled for this reason).
