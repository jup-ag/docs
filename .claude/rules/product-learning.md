...
- Live API: `https://api.raccoons.dev/gacha/v2` (base URL per Thomas, 2026-09-03; the gateway is assumed to map `/gacha/v2/<path>` onto the service's `/v2/<path>`).
- OpenAPI spec in this repo: `openapi-spec/gacha/gacha.yaml` (v2 paths only, `/v2` prefix stripped, hand-patched; see below).
- Source code: `jup-ag/gacha-api` (Hono + `@hono/zod-openapi`). `docs/openapi.json` there is generated by `pnpm openapi:sync` and CI-enforced in sync. Docs written against commit `ac49b04`.
- First-party guides that were ported: `gacha-api/docs/{index,guides/conventions,guides/packs,guides/transactions,guides/platforms}.mdx`.

## Undocumented Behavior

- [2026-09-03] `/v2` needs no API key at all (no `securitySchemes` in the generated spec; `x-api-key` is only checked on the legacy `/v1` routes). The docs copy declares `security: []`. Source: `gacha-api/http/app.ts`.
- [2026-09-03] Money-moving routes (purchase, open, buy, list, delist, buyback, submit) sit behind a `geoBlock` middleware that returns `451 region_restricted` and fails open when the geo lookup errors. Source: `gacha-api/http/middleware/geo-block.ts`.
- [2026-09-03] The `409 sold_out` / `409 inactive` codes are not literal `AppError` codes; they come from `MachineUnsellableError.reason` re-thrown as `AppError(409, err.reason, ...)` in `routes/v2/machines.ts`. Both are legitimate to document.
- [2026-09-03] Phygitals buyback quotes are throttled upstream to 1 request/second, so an inventory page only quotes its first few items; the rest carry `buyback: null` until a later request warms the cache. Source: `gacha-api/docs/guides/platforms.mdx`.
- [2026-09-03] Beezie is read-only (machines + chase inventory). It authenticates pulls with the end user's own session token, so no server-brokered purchase exists.

## Patterns & Conventions

- [2026-09-03] Platform ids are hyphenated (`collector-crypt`, `phygitals`, `beezie`). Machine ids are `${platform}:${externalId}` (e.g. `collector-crypt:pokemon_50`). Pack slugs are Jupiter's own and carry no platform. Assets are bare mint addresses; the platform is inferred from the whitelisted collection.
- [2026-09-03] Two pagination envelopes coexist: offset `{ total, page, limit, hasMore, items }` (page 1-based, limit max 100) on inventory/marketplace routes, and keyset `{ items, nextCursor }` (limit max 100, default 25) on `/users/{wallet}/activity` and `/latest-pulls` (the latter also takes `since`).
- [2026-09-03] Error envelope is `{ error: { code, message, requestId }, status, message }` with stable snake_case codes. Cache-control is only set on `200` GETs; every non-GET and every error is `no-store`.
- [2026-09-03] `POST /marketplace/{asset}/sell` is deprecated in favour of `POST /buyback/{asset}` and `POST /buyback`; it emits `Deprecation`/`Sunset` headers. The wrapper page carries `deprecated: true`.

## Known Issues

- [2026-09-03] The generated spec has `tags: null`, a `localhost` server, no `security`, and three operations without a `description` (`GET /machines/{id}`, `GET /machines/{id}/inventory`, `GET /transactions/{reference}`). All are patched in the docs copy only (scratch conversion script, not committed). Worth fixing upstream in `gacha-api` so a future re-sync carries them. `platform` query parameters are plain strings upstream, not enums.
- [2026-09-03] The upstream descriptions use em dashes and American spelling; the docs copy replaces them (commas/colons, British spelling). Re-apply on any re-sync.

## Content Gaps

- [2026-09-03] Gacha is not public yet, so every page is `hidden: true`, absent from `docs.json` and `llms.txt`. When it launches: add a Docs-tab `menu` item and an API reference group, un-hide the pages, regenerate `llms.txt`, and post a changelog entry in `developer-platform`.
