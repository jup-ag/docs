...
- **OpenAPI spec:** `openapi-spec/trigger/v2/trigger.yaml` in this repo. The price-order surface was complete; the DCA paths were added 2026-06-26 from live + source. The hand-written source-repo doc (`docs/dca-api.md`) drifts from live (e.g. create response), so live wins.
...
- [2026-06-26] **Limits (prod):** min **$10 per round** (`MINIMUM_ORDER_AMOUNT_USD=10` in `api-v2/wrangler.toml`, 0.20 USD tolerance, validated against `inputAmount/orderCount`); `orderCount` 2–100; `intervalSeconds` 60–31,536,000; `beginFillAt` ≤ 30 days out; max **10 active** DCA orders per wallet. `retryWindowSeconds = clamp(intervalSeconds*0.5, 30, 7200)` (observed 30 for interval 60).
