...
- [2026-07-09] `tx.jup.ag` methods are role-gated. Verified with an external integrator API key: `sendTransaction` and `sendJitoBundle` are available to external callers; `sendAndConfirmTransaction` and `sendYoloModeTransaction` return `-1012 Access denied` for the external role (internal-only). Decided by Kyle/groovie (2026-07-09): advertise ONLY `sendTransaction` to integrators. `sendJitoBundle` is internal-teams-only (despite being reachable by an external key), and `sendAndConfirmTransaction` is scheduled for deprecation. So integrator docs cover `sendTransaction` alone.
