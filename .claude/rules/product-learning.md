...
- [2026-03-10] JWT authentication via challenge-response flow. Challenge types: `message` (standard wallets) or `transaction` (hardware wallets). Challenge TTL: 5 min. **Correction [2026-09-09] (BUILD-879):** the flat "JWT TTL: 24h / no refresh endpoint" is now only the DEPRECATED `access_only` mode — see the Auth refresh-token entries below.
...
## Auth — refresh tokens (`access_refresh` mode) — live-verified 2026-09-09 (BUILD-879)

- [2026-09-09] `POST /trigger/v2/auth/verify` now takes an optional **`authMode`** on BOTH `message` and `transaction` variants: `access_only` (default when omitted) | `access_refresh`. The challenge step (`/auth/challenge`) is unchanged. Internal tracker is ULT-6 ("short access + sliding refresh tokens"); the artifact's "SWAP-627 phase 4" removal tag is internal-only and deliberately kept OUT of public docs.
- [2026-09-09] **`access_only` (deprecated)** verify response: `{ authMode:"access_only", token:<jwt> }` with `exp−iat = 86400s` (24h), returned WITH headers `Deprecation: @1784592000` (RFC 8594 `@<unix-seconds>` form; ~2026-07-20, matches the "deprecated 2026-07-21" tag) and `Link: </v2/auth/refresh>; rel="successor-version"` (note: the raw header path has no `/trigger` prefix — that's gateway-stripped; docs use the full `/trigger/v2/...` path). **`access_refresh`** verify response: `{ authMode:"access_refresh", accessToken:<jwt, exp−iat=900s / 15m>, refreshToken:"<familyUuid>.<opaqueSecret>", expiresAt:<ISO8601 ms>, tokenType:"Bearer" }`, NO deprecation headers. All live-verified with a throwaway ed25519 key (message signing; no funds/on-chain needed).
- [2026-09-09] **`POST /trigger/v2/auth/refresh`** `{ refreshToken }` → 200 `{ accessToken, refreshToken, expiresAt, tokenType }` (same shape as access_refresh verify but **no `authMode` field**). Rotates the secret each call, keeping the family-uuid prefix. A verified 401 path: reusing a refresh token after `/logout` returns `401 {"error":"Unauthorized"}`. **Grace confirmed live:** an immediate repeat of the same `/refresh` request returned 200 (tolerated retry). The exact "~90s" grace window + "reuse-after-grace revokes the whole family" are from the source-migration artifact (ULT-6), NOT independently timed here (gateway rate-limits at ~10 req/window made precise timing impractical); documented functionally without asserting a hard SLA number. `/auth/refresh` takes only ApiKeyAuth (the refresh token is the credential; no Bearer).
- [2026-09-09] **`POST /trigger/v2/auth/logout`** `{ refreshToken }` — **keyless (no Bearer)** (ApiKeyAuth only, no `Authorization` needed), always 200 `{ "success": true }`; the refresh token is then revoked (subsequent `/refresh` → 401). **`POST /trigger/v2/auth/logout-all`** requires `Authorization: Bearer <accessToken>` (empty `{}` body) → 200 `{ "success": true }` and invalidates every refresh family for the wallet (bumps a per-wallet cutoff); called without a Bearer → 401. Both live-verified.
- [2026-09-09] **Source clone is stale, live is authority.** The local `jup-ag/trigger-order` clone's `origin/main` ref is stuck at 2026-03-09 (`98a33a6`; SSH key unavailable to fetch fresh) and predates the whole feature — it still has `TOKEN_TTL_MS = 24h` and no `authMode`/refresh routes, and there is NO `src/service/refresh/` dir, so the artifact's exact file:line pointers (`schema.ts:37-50`, `service/refresh/config.ts`, etc.) could NOT be confirmed and are partly fabricated. Everything documented was verified against the LIVE gateway instead (authority order: live > source). Re-verify the grace window and any `authMode` default change against a fresh checkout when SSH access is restored.
