...
## /order Error Contract

- [2026-09-15] `/order` error responses are **HTTP 200 with `transaction: ""`** and carry `errorCode`, `errorMessage`, and `error` (byte-identical to `errorMessage`) alongside valid pricing. Verified live: metis `1` -> `Insufficient funds`; metis `3` -> `Minimum $5 for gasless` (JUP input) and `Minimum $10 for gasless` (W input) within the same minute; jupiterz `1` -> `Insufficient balance to fund the swap`; jupiterz `2` -> `Missing associated token account`. The dollar figure is computed per request from the swap's USD value and the route's rent + priority fee cost (`swap-api/src/metis/quote/gasless.ts`), so it is not a constant and can move without a deploy. jupiterz `3` (`Quote could not be built into a transaction`) is in source but was not reproduced live (needs a failed simulation). Repro script: `~/Dev/jup-workbench/swapv2-order-error-codes.cjs`.
- [2026-09-15] **Aggregator `errorCode: 2` is not returned by the live API.** The constant exists (`swap-api/src/errors/registry.ts`) but has zero call sites: it lost its call site in `ultra-api` in Nov 2025 when the SOL pre-check was replaced by simulation, and `swap-api` was forked after that. Not reproduced in ~30 live probes. The source string is `Top up 0.01 SOL for gas`; **"Insufficient SOL for gas" exists only in our docs and was never in the code.** Docs still publish the row (marked "Not currently returned") pending an owner call on retired vs orphaned. See BUILD-881.
- [2026-09-15] JupiterZ `errorCode: 4` (`Token account is frozen`) exists in source but is intercepted before it can be returned. Deliberately undocumented.
- [2026-09-15] `/order` has a second failure shape with **no `errorCode` at all**: HTTP 400 with `{requestId, error}`, e.g. `{"error":"Failed to get quotes"}`, which covers more than one condition. Integrators that read `errorCode` unconditionally break on this path. Also observed for a request with an unfunded `payer`.
- [2026-09-15] **`/swap/v2/order` is served by `swap-api`, not `ultra-api`** (`ultra-api` serves `/ultra/v1/order`). `swap-api` was forked from `ultra-api` and the two error tables are kept in sync **by hand**, with no shared package, so V1/V2 message parity is current behaviour and not a guarantee. The aggregator string `Insufficient funds` was previously `Taker has insufficient input` and was changed with no deprecation notice, which is why docs tell integrators to key off `router` + `errorCode`.
