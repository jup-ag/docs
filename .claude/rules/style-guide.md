...
- [2026-07-02] Lend AMM naming: the product is the **Lend AMM** ("Jupiter Lend AMM" only where the Jupiter context isn't already established). Nav group "Lend AMM"; page titles "AMM Overview", "AMM SDK", "AMM APIs", "AMM CPI Integration", "AMM Errors". Do NOT brand the product "DEX": "DEX" is reserved for the on-chain program (the DEX program, `jupZ4m2…`) and code identifiers (`Dex` account, `dex_id`, `DexOperation`, `Dex*` errors). File paths stay `lend/dex/`.
