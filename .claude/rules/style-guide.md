...
- [2026-08-14] Integrator-facing Metis routing pages say "AMM", not "DEX" (YY, BUILD-812): page title "Integrate AMM into Metis", nested nav group "Integrate AMM into Metis". "DEX" survives in code identifiers only (see the 2026-08-25 slug entry: the file path is now `swap/routing/amm/integration`).
- [2026-08-25] Routing-integration URL slugs are version-scoped and product-scoped (BUILD-812): `swap/routing/amm/*` for Metis AMM integration (`amm/integration`, `amm/market-listing`) and `swap/routing/rfq/v{1,2}/*` for JupiterZ MM integration (`rfq/v1/overview`, `rfq/v1/webhook-api`, `rfq/v1/quoting`, `rfq/v1/settlement`, `rfq/v2/streaming`). Chosen so future versions/pages slot in without URL changes (V3 → `rfq/v3/*`). The V1 webhook page was split into a landing ("Webhook Integration (V1)") plus "Webhook API", "Quoting and fills", and "Settlement and special cases"; V2 stays one page ("Streaming Integration (V2)"). All old slugs (`dex-integration`, `market-listing`, `rfq-integration`, `rfq-streaming`) redirect to the new URLs.
- [2026-09-24] Page titles are unique site-wide and carry their product (BUILD-807/808). `title` renders as `<title>`, H1, og:title and the llms.txt label, so it must identify the page without the sidebar: `Create Order (Trigger API)`, `Get Positions (Portfolio API)`, `Earn Positions`. Pattern: guide pages `Action (Product)`, API reference pages `Action (Product API)`, unmaintained versions `Action (Product V1)` / `Action (Product V1 API)`. Keep the sidebar short with `sidebarTitle` (the old short title) whenever the new `title` is longer. Product landing pages are named for the search intent, not "Overview"/"About X": `Solana Swap API`, `Solana Price API`, `Solana Token API`, `Prediction Market API`, with `sidebarTitle` keeping the nav label. Never set `og:title`, `twitter:title`, `description` or `og:description` in `docs.json` `seo.metatags`: global values override every page (that is what rendered "Jupiter Developers" as the `<title>` of all 233 pages until 2026-09-24). Site name is `Jupiter Developers`, so titles render as `Page Title - Jupiter Developers`.
