...
* New Real-Time Slippage Estimation (RTSE) documentation covering how Jupiter estimates and protects against slippage
...
<Card title="Developer Platform Docs" href="/portal/setup" icon="key" iconType="solid" horizontal/>
...
Start with `/order`. It gives the best price because all routers compete, including JupiterZ market makers who often beat onchain routing on major pairs. Only use `/build` if you need to modify the transaction.

Ultra and Metis APIs remain available. New integrations should use the Swap API, and existing integrations are encouraged to migrate when convenient. See the [migration guide](/swap/migration) for details.

- [Swap API overview](/swap)
- [Order & Execute](/swap/order-and-execute)
- [Build Custom Transactions](/swap/build)
...
## Jupiter Lend Documentation

Comprehensive documentation for Jupiter Lend (powered by Fluid Protocol) is now available, covering the full Lend product suite.

* **Earn:** Deposit and withdraw via the Lend SDK or REST API, with read-data helpers for jlToken shares and balances
* **Borrow:** Create positions, deposit collateral, borrow, repay, and withdraw, including combined single-transaction operations
* **Flash Loans:** Borrow and repay within the same transaction for arbitrage, liquidation, or collateral swaps
* **Liquidity & Analytics:** Reserve data, utilisation curves, and on-chain analytics queries
* **Advanced Recipes:** Multiply (leveraged long), Unwind (deleverage), Repay-with-Collateral, and Vault Swap, all using CPI + Jupiter swap in a single transaction
* **Architecture:** Three-program design (Liquidity Layer, Lending/Earn, Vaults/Borrow) with oracle integration (Pyth, SwitchboardV2, ScopePrices)

<Card title="Lend Documentation" href="/lend" icon="building-columns" iconType="solid" horizontal/>
