...
Start with the Meta-Aggregator (`/order`). It gives the best price because all routers compete, including JupiterZ market makers who often beat onchain routing on major pairs. Only use the Metis Router (`/build`) if you need to modify the transaction.
...
- [Meta-Aggregator (Order & Execute)](/swap/order-and-execute)
- [Metis Router (Build)](/swap/build)
