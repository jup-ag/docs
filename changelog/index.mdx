...
## Swap API: Referral Fees and JupiterZ Routing

Referral fees on `/swap/v2/order` no longer disable JupiterZ routing. Integrations using `referralAccount` and `referralFee` can still receive RFQ quotes when JupiterZ is the best eligible route.

Integrator-sponsored gas still changes router eligibility: setting `payer` to a wallet different from `taker` restricts routing to Metis.
