...
description: "Query market state, interest rates, and utilisation for Jupiter Lend."
llmsDescription: "How to use the @jup-ag/lend-read SDK Liquidity module for analytics, dashboards, and APY aggregators. Covers listedTokens() for market discovery, getOverallTokenData() for supply/borrow rates and utilisation, getExchangePricesAndConfig() and calculateExchangePrice() for real-time price computation, getUserSupplyData() and getUserBorrowData() for individual positions, getUserMultipleBorrowSupplyData() for batched portfolio fetching, and getAllUserPositions() for global protocol state indexing."
---
