...
llmsDescription: "How to create a borrow position, deposit collateral, and borrow assets from Jupiter Lend vaults using Privy embedded wallets. Covers PrivyClient setup, getInitPositionIx and getOperateIx from @jup-ag/lend/borrow, versioned transactions with address lookup tables, Privy signing via privy.wallets().solana().signTransaction, and the full create-deposit-borrow flow. Complete guide for keyless borrowing."
---
...
<Steps>
  ...
  <Step>
    ...
    <Tip>
      The borrowed tokens are now in the Privy wallet. Repay with the standard [Repay](/docs/lend/borrow/repay) flow using the same `positionId`.
    </Tip>
  </Step>
</Steps>
...
<CardGroup cols={2}>
  <Card title="Deposit to Earn with Privy" icon="wallet" href="/docs/lend/privy/earn-deposit">
    Earn yield with Privy wallets
  </Card>
  <Card title="Deposit Collateral" icon="building-columns" href="/docs/lend/borrow/deposit">
