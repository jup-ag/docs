...
| `oracle`           | **Type:** [`OracleParams`](#oracleparams)<br/><br/> Contains data for the price oracle used for the custody.                                                                                             |
| `pricing`          | **Type:** [`PricingParams`](#pricingparams)<br/><br/> Contains data for the custody's price-related logic.                                                                                               |
| `permissions`      | **Type:** [`Permissions`](#permissions)<br/><br/> A set of global flags that can be set by the protocol's administrator to enable or disable trade actions which is useful during program upgrades or black swan events. |
...
| Field             | Description                                                                                                                                                          |
| :---------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `oracleAccount`   | **Type:** `publicKey` <br/><br/> The public key of the oracle account used to fetch the custody's token price.                                                       |
| `oracleType`      | **Type:** `OracleType` <br/><br/> The type of oracle used for the custody's price feed.                                                                              |
| `buffer`          | **Type:** `u64` <br/><br/> A buffer applied to the oracle price to account for price fluctuations or latency between price updates.                                  |
| `maxPriceAgeSec`  | **Type:** `u32` <br/><br/> The maximum age (in seconds) of an oracle price before it is considered stale. Transactions will fail if the oracle price exceeds this age. |
...
| Field                       | Description                                                                                          |
| :-------------------------- | :--------------------------------------------------------------------------------------------------- |
| `allowSwap`                 | **Type:** `bool` <br/><br/> Enables or disables swap operations for the custody.                     |
| `allowAddLiquidity`         | **Type:** `bool` <br/><br/> Enables or disables adding liquidity to the pool for the custody.        |
| `allowRemoveLiquidity`      | **Type:** `bool` <br/><br/> Enables or disables removing liquidity from the pool for the custody.    |
| `allowIncreasePosition`     | **Type:** `bool` <br/><br/> Enables or disables opening or increasing positions for the custody.     |
| `allowDecreasePosition`     | **Type:** `bool` <br/><br/> Enables or disables closing or decreasing positions for the custody.     |
| `allowCollateralWithdrawal` | **Type:** `bool` <br/><br/> Enables or disables collateral withdrawals for the custody.              |
| `allowLiquidatePosition`    | **Type:** `bool` <br/><br/> Enables or disables liquidations for the custody.                        |
