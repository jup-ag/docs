...
- **Claim Winnings**: Receive USDC payouts when your predictions are correct
...
5. **Winners claim payouts** - each winning contract is worth $1 (no fees)
...
- **Status Flags**: `isLive`, `isActive`, `isTrending`, `isRecommended`
...
- **Timestamps**: `openTime`, `closeTime`, `settlementTime`
...
**Vault Functions:**
- **Escrow Management**: Holds deposited USDC during active trading
...
5. **Users Claim**: Winners receive $1 per contract (minus fees)
...
The system charges fees on trading activity:
- **Underlying Protocol Fees**: Integrated protocols have fees
- **Platform Fees**: Zero platform fees (Jupiter takes 0% of the trading volume)
- **Total Fee**: Displayed before order execution
- Fees reduce net P&L but are known upfront
