...
{/* <!-- TO DO: Document behavior when minSellPriceUsd cannot be met --> */}
{/* ### Price Protection with minSellPriceUsd

The `minSellPriceUsd` parameter protects you from selling at unfavorable prices. If the market price falls below your minimum, the order may be partially filled or fail.

<Warning>
**Slippage Protection**

Always set a reasonable `minSellPriceUsd` based on current market conditions. Check the position's `sellPriceUsd` field to see the current available sell price.
</Warning> */}
