...
llmsDescription: "POST /orders creates buy orders for YES/NO contracts. Required parameters: ownerPubkey, marketId, isYes, isBuy, contracts, maxBuyPriceUsd. Returns base64 transaction to sign and submit to Solana."
---
...
This guide walks through creating a buy order to open a position in a prediction market. You'll learn how to construct the order request, sign the returned transaction, and submit it to the Solana network.
...
| `maxBuyPriceUsd` | string | No | Maximum price per contract in native token units |
| `depositAmount` | string | Yes | Amount to deposit (in native token units) |
| `depositMint` | string | Yes | Token mint for deposit |

<Info>
**Prices in native token units**

The `maxBuyPriceUsd` parameter uses native token units where **1,000,000 native token units = \$1.00**.

Examples:
- `700000` = \$0.70 (700000 / 1000000 = \$0.70)
- `500000` = \$0.50 (500000 / 1000000 = \$0.50)
- `100000` = \$0.10 (100000 / 1000000 = \$0.10)
</Info>
...
The total cost of an order depends on:
- **Contract price**: Current market price (capped by your `maxBuyPriceUsd`)
- **Number of contracts**: How many you're buying
- **Fees**: Platform and venue fees

For example, buying 10 YES contracts at $0.65 each:
- Contract cost: 10 x $0.65 = $6.50
- Estimated fees: ~1% = $0.065
- **Total**: ~$6.565

<Warning>
**Price Protection**

If the market price exceeds your `maxBuyPriceUsd`, the order may be partially filled or fail. Set a reasonable max price based on current market conditions.
</Warning> */}
