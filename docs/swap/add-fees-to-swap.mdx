...
```
  ...
  Using the above, we will now know the `feeAccount` to be passed in as the parameter in Legacy Swap API. You can refer to the [Build Swap Transaction](/docs/swap/build-swap-transaction) guide to add any parameters where necessary to help transaction sending, etc.
