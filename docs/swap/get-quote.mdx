---
title: "Get Quote"
description: "Get a swap route quote from the Metis routing engine."
llmsDescription: "GET /swap/v1/quote returns route plans from the Metis routing engine. Required params: inputMint, outputMint, amount, slippageBps."
---
...
The quote response includes `mostReliableAmmsQuoteReport`, which shows what the most reliable AMMs would have quoted for the same token pair. Each key in the `info` object is an AMM account address, and the value is the quoted output amount as a string.

This is useful for verifying that the routing engine selected the best path. Compare the reliable AMM quotes with the chosen route's `outAmount` to understand routing decisions. You can map AMM addresses to DEX labels using the [`/program-id-to-label`](/api-reference/swap/program-id-to-label) endpoint.
