---
title: "Routing"
description: "How routing works and what affects it"
llmsDescription: "How routing works on Jupiter Swap API V2. Covers the Juno routing architecture (Iris, JupiterZ RFQ, Dflow, OKX), which routers are available on each endpoint (/order uses all routers, /build uses Metis only, Swap V1 uses Metis only), the routing impact matrix showing which optional parameters disable which routers, and ultra vs manual mode. Integrators building DEXes can integrate into Metis; market makers can integrate into JupiterZ via RFQ."
---

The Swap API V2 routes through multiple liquidity sources. Which endpoint you use and which parameters you pass determines which routers are eligible, directly affecting the price you get.

## Routing architecture

Jupiter's routing is powered by **Juno**, a meta-aggregator that combines multiple routing engines and selects the best execution price across all of them. Juno employs a self-learning mechanism that automatically sidelines underperforming sources and continuously optimises across all engines.

| Engine | Role |
| --- | --- |
| **Iris** | Jupiter's latest onchain routing engine. Multi-hop, multi-split swaps across Solana DEXes. Evolved from Metis. |
| **JupiterZ** | RFQ (Request for Quote) system where market makers compete to provide off-chain liquidity. Often beats onchain by 5-20bps on major pairs. |
| **Metis** | The original low-level routing engine. Exposes raw swap instructions for full transaction control (CPI, custom instructions). Now an [independent public good](https://metis.builders). |
| **Dflow** | Third-party decentralised order flow. |
| **OKX** | Third-party OKX liquidity. |

## Router availability by endpoint

Not all routers are available on every endpoint. This is the most common source of confusion for integrators:

| Endpoint | Routers | Notes |
| --- | --- | --- |
| **`/swap/v2/order`** (no optional params) | Iris, JupiterZ, Dflow, OKX | All routers compete. Best price. |
| **`/swap/v2/order`** (with optional params) | Depends on params | See [routing impact matrix](#routing-impact-matrix) below. |
| **`/swap/v2/build`** | Metis only | Returns raw instructions. No RFQ, no third-party routers. |
| **Swap V1** (`/v6/quote` + `/v6/swap`) | Metis only | Legacy endpoint. Same Metis engine, no Juno aggregation. |

<Warning>
If you are using `/build` or Swap V1, you are only getting Metis routing. You will miss JupiterZ (RFQ) liquidity, which can be 5-20bps better on major pairs. Use `/order` without optional params for the best price.
</Warning>

## Routers

The `/order` endpoint, by default with no optional parameters, enables all routers:

| Router | Type | Description |
| --- | --- | --- |
| **Metis** (via Iris) | Onchain aggregator | Jupiter's core routing engine, accessed through Iris |
| **JupiterZ** | RFQ (Request for Quote) | Market makers providing off-chain liquidity |
| **Dflow** | Third-party | Decentralised order flow |
| **OKX** | Third-party | OKX liquidity |

<Note>
  The `/order` response `router` field indicates which router won: `iris` (Metis), `jupiterz`, `dflow`, or `okx`.
</Note>

## Routing impact matrix

Adding optional parameters to `/order` can restrict which routers are eligible. This table shows the impact:

| Parameter | Metis | JupiterZ (RFQ) | Dflow | OKX |
| --- | --- | --- | --- | --- |
| **(no optional params)** | Yes | Yes | Yes | Yes |
| `receiver` | Yes | **No** | Yes | Yes |
| `referralAccount` & `referralFee` | Yes | **No** | Yes | Yes |
| `payer` (integrator gasless) | Yes | **No** | **No** | **No** |
| `excludeRouters: jupiterz` | Yes | **No** | Yes | Yes |

**Key takeaway:** adding `receiver`, `referralAccount`, `referralFee` or `payer` disables JupiterZ (RFQ), which may result in worse pricing on major pairs where market makers often beat onchain routing by 5-20bps.

For the full parameter reference, see the [API reference](/api-reference/swap).

## Order mode

The `/order` response includes a `mode` field that tells you which routing behaviour was applied:

| Mode | Meaning | Routers |
| --- | --- | --- |
| **ultra** | No optional params used. All routers competed. Best price. | Metis, JupiterZ, Dflow, OKX |
| **manual** | Optional params detected. Routing may be restricted. | Depends on which params are set |

Use `mode` to verify your request got the routing behaviour you expected. If you see `"mode": "manual"` but didn't intend to restrict routing, check which optional parameters you are passing.

<Note>
This is similar to how our jup.ag frontend behaves, when you use custom settings like slippage, priority fee strategy or dex/router exclusions, you will get a different `mode`.
</Note>

## Integrate into routing

If you want your liquidity routed through Jupiter:

- **DEX operators**: [Integrate DEX into Metis](/docs/swap/routing/dex-integration) to get your markets included in Jupiter's onchain routing.
- **Market makers**: [Integrate into JupiterZ (RFQ)](/docs/swap/routing/rfq-integration) to provide off-chain liquidity via the RFQ system.

## Related

- [Fees](/docs/swap/fees) for fee models and how referral fees affect routing
- [Gasless Swaps](/docs/swap/advanced/gasless) for how `payer` affects routing
- [Order & Execute](/docs/swap/order-and-execute) for the default swap flow
- [API Reference: GET /order](/api-reference/swap/order) for the full parameter specification
