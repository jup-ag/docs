...
- We set the gas payer to the MM signer and upon receiving your partially signed transaction from `/execute`, we send it to the MM for them to sign and execute
...
- This needs to be used together with referral parameters (`referralAccount` and `referralFee`) because we assume that you will be taking fees to recoup the costs spent on gas
...
- [Routing](/docs/swap/v2/routing) for how `payer` affects routing
