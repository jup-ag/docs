...
- When a JupiterZ (RFQ) market maker provides the winning quote, the market maker pays network and priority fees directly. ATA rent is not covered by the market maker, so the taker must have enough SOL for rent or JupiterZ will not be routed.
...
- Routes through Metis (Iris) only

### On `/build`

Pass `payer` to change the fee payer on all returned instructions. The payer address replaces the default `userPublicKey` as the fee payer for the transaction.
...
## Scenario matrix (`/order`)

| Scenario | Automatic gasless | RFQ (if quoted) |
| --- | --- | --- |
| **Taker has SOL and ATA** | Taker pays gas | MM pays gas |
| **Taker has SOL, no ATA** | Taker pays gas and ATA rent | MM pays gas, taker pays ATA rent |
| **No SOL, has ATA** | Gas taken from swap amount (min \$10 swap) | MM pays gas |
| **No SOL, no ATA** | Gas and rent from swap amount (min \$10 swap) | Not supported (no ATA funding) |
| **No SOL, has ATA, small swap** | Quote shown but cannot swap | MM pays gas |
| **No SOL, no ATA, small swap** | Quote shown but cannot swap | Not supported (no ATA funding) |
...
- [Parameters & Routing](/docs/swap/v2/parameters) for how `payer` affects routing
- [Fees](/docs/swap/v2/fees) for how gasless affects the fee calculation
