...
const blockhashStr = bs58.encode(Buffer.from(blockhash)); // Convert byte array to base58
...
const transactionMessage = pipe(
  ...
  (tx) =>
    setTransactionMessageLifetimeUsingBlockhash(
      { blockhash: blockhashStr, lastValidBlockHeight: BigInt(lastValidBlockHeight) },
      tx
    ),
...
const { blockhash, lastValidBlockHeight } = build.blockhashWithMetadata;
const recentBlockhash = bs58.encode(Buffer.from(blockhash)); // Convert byte array to base58
...
See [Swap Instructions](/docs/swap/v2/build/other-instructions) for a reference of common instructions.
...
- [Swap Instructions](/docs/swap/v2/build/other-instructions) for common instructions to compose with your swap
