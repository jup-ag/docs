...
<CodeGroup>
```typescript @solana/kit expanadable
import {
  AccountRole,
  Address,
  address,
...
// Step 2: Collect instructions (without compute unit limit — we'll simulate first)
...
See [Other Instructions](/docs/swap/v2/build/other-instructions) for a reference of common instructions.
...
The code example above demonstrates this in steps 4-5 (kit) or is handled automatically by the compute budget instructions (web3.js). See [Solana fee structure](https://solana.com/docs/core/fees/fee-structure) for more on compute units and [Estimate Compute Units](/docs/swap/v2/advanced/compute-units) for a standalone guide.
...
For the full parameter reference, see the [API reference](/api-reference/swap). For how parameters affect routing, see [Routing](/docs/swap/v2/routing).

## Related

- [Other Instructions](/docs/swap/v2/build/other-instructions) for common instructions to compose with your swap
