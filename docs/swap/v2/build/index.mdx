...
The kit code example above demonstrates this in steps 4-5 (simulate with max CU, then rebuild with 1.2x the simulated value). The web3.js example includes `computeBudgetInstructions` as-is, which only sets the compute unit price. For production use, you should also simulate and set the compute unit limit in your web3.js code. See [Solana fee structure](https://solana.com/docs/core/fees/fee-structure) for more on compute units and [Estimate Compute Units](/docs/swap/v2/advanced/compute-units) for a standalone guide.
