...
llmsDescription: "GET /tokens/v2/content endpoints for retrieving curated token content, trending tokens, paginated feeds, and summaries powered by Jupiter VRFD. Pro tier only."
---
...
For complete API documentation including request/response schemas, parameters, and examples, refer to the [Content API Reference](/api-reference/tokens/get-content).
...
<Tip>
  ...
  * Discover content for trending tokens on Jupiter
  ...
  Refer to [Content API Reference](/api-reference/tokens/get-content) for full schema.
</Tip>
