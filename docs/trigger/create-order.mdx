...
```
1. Register vault     →  GET /v2/vault/register (first time only)
2. Craft deposit      →  POST /v2/deposit/craft
3. Sign deposit tx    →  (client-side)
4. Create order       →  POST /v2/orders/price (with signed tx)
```

## Step 1: Register a Vault

On first use, register a vault for your wallet. This creates a Privy-managed custodial account that holds deposits for your orders.

```typescript
const vaultResponse = await fetch('https://api.jup.ag/trigger/v2/vault/register', {
  headers: {
...
If you already have a vault, use `GET /v2/vault` to retrieve it.
...
```typescript
const transaction = deserializeTransaction(deposit.transaction);
const signedTransaction = await wallet.signTransaction(transaction);
const depositSignedTx = base64Encode(signedTransaction.serialize());
```
