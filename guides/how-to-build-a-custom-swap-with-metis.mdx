...
| `otherAmountThreshold` | Minimum acceptable output — `outAmount` minus `slippageBps` tolerance |
...
Errors can surface at every stage of the swap flow. Handle them accordingly.
...
| DEX program errors | A CPI-ed AMM returned an error during the swap | If the AMM's IDL is public, decode the error code on a block explorer like [Solscan](https://solscan.io). If the error is obscure, reach out on [Jupiter Discord](https://discord.gg/jup) |
...
When something fails, log the entire flow so you can reproduce and diagnose the issue:
...
<Tip>
If a transaction fails during local simulation (preflight), log the base64-encoded `swapTransaction` for debugging. But prefer submitting on-chain with `skipPreflight: true` — block explorers like [Solscan](https://solscan.io) capture account state on failed transactions, which is far more useful for diagnosis.
</Tip>

### Retrying

If a swap fails, re-quote and try again. Quotes go stale quickly — never reuse a previous quote.

```javascript
// On failure, simply re-quote and retry
const signature = await swap(inputMint, outputMint, amount, slippageBps);
```
