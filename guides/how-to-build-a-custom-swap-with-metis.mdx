...
<Warning>
Always re-quote before retrying. Reusing a stale quote leads to slippage failures or expired transactions.
</Warning>
...
async function swapWithRetry(maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      const quoteResponse = await getQuote();  // Always re-quote
