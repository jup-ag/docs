...
<AccordionGroup>
  <Accordion title="During quoting">
    ...
    |-------|---------------|------------|
    | `NO_ROUTES_FOUND` / `COULD_NOT_FIND_ANY_ROUTE` | No liquidity path exists | Check if the token is tradeable on [jup.ag](https://jup.ag). See [Market Listing](/swap/routing/market-listing) for the grace period and liquidity criteria |
