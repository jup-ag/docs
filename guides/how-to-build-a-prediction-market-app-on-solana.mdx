...
llmsDescription: "Jupiter Prediction API: REST API for binary prediction markets on Solana. Trade YES/NO outcomes on real-world events (sports, crypto, politics). Base URL: https://api.jup.ag/prediction/v1. Supports Polymarket and Kalshi data. Returns unsigned transactions for order creation, position management, and payout claims."
---

## TL;DR

Use Jupiter's Prediction API to let users trade on real-world event outcomes (sports, crypto, politics, etc.) directly on Solana. The API handles order matching, position tracking, and settlement. Base URL: `https://api.jup.ag/prediction/v1`
