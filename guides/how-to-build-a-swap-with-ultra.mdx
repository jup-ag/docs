...
<Warning>
Ultra V1 is deprecated. Use [Swap API V2](/docs/swap/index) instead - it unifies Ultra and Metis into a single API with `/order` (managed execution) and `/build` (custom transactions). See the [Migration Guide](/docs/swap/migration).
</Warning>
...
Done. Your swap lands in 50-400ms via [Jupiter Beam](/blog/ultra-v3#sub-second-transaction-landing).
...
| `referralAccount` | No | Referral account for [integrator fees](/docs/ultra/add-fees-to-ultra) |
| `referralFee` | No | Referral fee in basis points (50-255) |
| `excludeRouters` | No | [Manual mode](/docs/ultra/gasless) only. Comma-separated routers to exclude: `iris`, `jupiterz`, `dflow`, `okx` |
| `excludeDexes` | No | [Manual mode](/docs/ultra/gasless) only. Comma-separated DEXs to exclude (applies to Iris router only). [Full list of DEXs](/api-reference/swap/program-id-to-label). |
...
For the full list of error codes including aggregator and RFQ-specific codes, see [Response docs](/docs/ultra/response).
...
Check `gasless: true` in the order response to know if it kicked in. For the full breakdown of scenarios, see [Gasless Support](/docs/ultra/gasless).
...
Fees range from 50 to 255 bps. Jupiter takes 20% of your integrator fee (no additional Ultra base fee on top). Ultra decides which token to collect fees in based on a [priority list](/docs/ultra/fees#priority-list). If the referral token account for that mint isn't initialised, the swap still goes through, but without your fee. So always check that `feeBps` matches what you set.

Full setup walkthrough: [Add Integrator Fees](/docs/ultra/add-fees-to-ultra).
...
Full breakdown: [Fees](/docs/ultra/fees).
...
<AccordionGroup>
  ...
  <Accordion title="What's the rate limit?">
    Dynamic. Base is 50 requests per 10-second sliding window, and it scales up with your swap volume. $1M in daily volume gets you ~165 req/10s. No paid tiers. Just get a free API key at [portal.jup.ag](https://portal.jup.ag). Details: [Rate Limits](/docs/ultra/rate-limit).
  </Accordion>
...
- **[Gasless Support](/docs/ultra/gasless).** Deep dive into gasless mechanisms.
- **[Add Integrator Fees](/docs/ultra/add-fees-to-ultra).** Set up referral accounts and earn on swaps.
- **[Embed Swap Widget](/guides/how-to-embed-a-swap-widget).** Want a drop-in UI instead of an API? Use Plugin.
- **[Ultra V3 Blog](/blog/ultra-v3).** What makes Ultra the best execution engine on Solana.
