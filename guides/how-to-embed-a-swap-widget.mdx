...
<body>
  ...
  <script>
    window.onload = function() {
      window.Jupiter.init({ displayMode: "widget" });
    };
  </script>
</body>
</html>
...
Open this in a browser and you have a floating swap widget. That's it.

---

## When you need this

You want to add swap functionality to your app without building any of it yourself:

- **dApp with swap** — Your app needs token swaps but you don't want to build the UI, handle transactions, or run an RPC node
- **Community token site** — Force users to buy your token by locking the output mint with `fixedMint`
- **Existing app with a wallet** — Your app already has a wallet provider — Plugin supports wallet passthrough, so no extra wallet infrastructure needed
- **Payment flow** — Accept a specific token at a fixed amount using `fixedAmount` and `fixedMint`
...
The minimal approach — no build tools, no dependencies:
...
Add the plugin script to your `public/index.html`:
...
Create a `plugin.d.ts` file in your project:
...
For the full type declaration, see the [plugin repository](https://github.com/jup-ag/plugin/blob/main/src/types/index.d.ts).
</Accordion>
...
<Info>
Next.js requires `"use client"` for components that use `window` or `useEffect`. The `Script` component with `onReady` ensures the plugin initialises after the script loads.
</Info>

### Alternative: npm package

You can also install the plugin as an npm package instead of using the script tag:
...
<Warning>
Using the npm package means you maintain its dependencies and update it manually. The script tag always loads the latest version.
</Warning>
...
**Community token site example** — force users to buy your token:
