...
Save this as `swap.html` and serve it over HTTP (e.g. `npx http-server`), then open `http://localhost:8080/swap.html`. You have a working swap widget. That's it.

<Note>
Wallet extensions like Phantom and Backpack require an HTTP origin to connect. Opening the file directly (`file://`) will load the widget but wallets won't be able to connect. Always serve over `http://` or `https://`.
</Note>

<Accordion title="New to coding? Follow these steps">
You need Node.js installed from [nodejs.org](https://nodejs.org) to run a local server.

1. Open any text editor (Notepad on Windows, TextEdit on Mac — switch to plain text mode first)
2. Copy the HTML code above and paste it into the editor
3. Save the file as `swap.html` (make sure it ends in `.html`, not `.txt`)
4. Open a terminal (Terminal on Mac, Command Prompt on Windows) and navigate to the folder where you saved the file
5. Run `npx http-server` — this starts a local web server
6. Open `http://localhost:8080/swap.html` in your browser
7. You should see a swap widget embedded in the page

That's a working swap widget running locally on your machine. To put it on the internet, upload the file to any web hosting service.
</Accordion>
...
The [Plugin Playground](https://plugin.jup.ag/) is a live sandbox where you can configure every aspect of the widget and see the results instantly. Switch display modes, set token defaults, tweak colours to match your brand, and copy the generated code straight into your project.
