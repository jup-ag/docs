...
<Info>
  The Borrow API exposes vault and position reads (`vaults`, `positions`) and operate transactions (`operate`, `operate-instructions`). See the [Borrow API](/lend/borrow/api). The Read SDK remains available for richer on-chain vault and position data.
</Info>
...
Borrow API and SDK flows for create-position, deposit, borrow, repay, withdraw, and liquidate are documented in the [Borrow operations](/lend/borrow) pages. For the REST flow, see the [Borrow API](/lend/borrow/api).
