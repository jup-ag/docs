...
All borrow endpoints accept an optional `market` parameter: `main` (default) or `ethena`. Pass it as a query parameter on the read endpoints (`?market=ethena`) or in the body on operate calls. Omitting it uses the `main` market.
...
- **Depositing WSOL collateral:** wrap SOL into your WSOL token account first. Without a funded WSOL account, the deposit fails with a token-program `insufficient funds` error.
...
// 2. Create position + deposit collateral via /operate (positionId: 0)
//    For WSOL vaults, wrap SOL into your WSOL token account first.
const deposit = await (
...
// 3. Borrow against the position via /operate-instructions
const borrow = await (
