...
<Warning>
  **WSOL collateral vaults require wrapped SOL.** For vaults whose `supplyToken` is `So11111111111111111111111111111111111111112` (WSOL), the operate transaction does not wrap native SOL for you. Wrap SOL into your WSOL token account before depositing, otherwise the transaction fails with a token-program `insufficient funds` error.
</Warning>
