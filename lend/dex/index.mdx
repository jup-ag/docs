...
The AMM's on-chain program is the **DEX program**. Prose and account names below use DEX where they refer to this program.
...
The AMM's own PDAs are derived against the **DEX** program id; the Liquidity Layer accounts a swap touches are derived against the **Liquidity Layer** program id. Seeds (all strings are ASCII bytes; `dex_id` is a little-endian `u16`):
...
```mermaid
flowchart TB
  AMM["AMM pool<br/>token0 / token1, center price, ranges,<br/>fee, shares, oracle state"]
