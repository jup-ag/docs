...
<Accordion title="Via Referral Program (Required only for Trigger API)">
  ...
  **1. Set up**
  ...
  You will need to complete the prerequisites and understanding of [Environment Setup](/docs/environment-setup) and [Get Quote and Swap](/docs/swap-api/get-quote) guide as this is reliant on the Legacy Swap API.
...
console.log(JSON.stringify(quoteResponse, null, 2));
  ...
  * If the token account is found, it will proceed to the Legacy Swap API.
...
```
  ...
  Using the above, we will now know the `feeAccount` to be passed in as the parameter in Legacy Swap API. You can refer to the [Build Swap Transaction](/docs/swap-api/build-swap-transaction) guide to add any parameters where necessary to help transaction sending, etc.
...
Finally, the user can sign the transaction and it can be submitted to the network to be executed. You can refer to the [Send Swap Transaction](/docs/swap-api/send-swap-transaction) guide to complete this step.
