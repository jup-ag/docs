...
<Tip>
  **API REFERENCE**
  ...
  To fully utilize the Ultra API, check out the [Ultra API Reference](/api-reference/ultra/execute).
</Tip>
...
<Note>
  **`/execute` PARAMETERS**
  ...
  Both required parameters are found in the [order response](/docs/ultra/get-order#order-response), do note that the `transaction` field is the base64 encoded transaction that [you need to sign](#sign-transaction) before submitting to the network.
</Note>
