...
description: "The Jupiter Recurring API enables you to create automated recurring orders on Solana, allowing users to set up regular token swaps that execute automatically based on time intervals or price conditions."
---
...
1. [**Create Order**](/recurring-api/create-order): Create a new recurring order with your desired parameters.
2. [**Cancel Order**](/recurring-api/cancel-order): Cancel an existing recurring order.
3. [**Deposit in Price-based Orders**](/recurring-api/deposit-price-order): Deposit funds in price-based orders.
4. [**Withdraw from Price-based Orders**](/recurring-api/withdraw-price-order): Withdraw funds from price-based orders.
5. [**Get Recurring Orders**](/recurring-api/get-recurring-orders): Retrieve the history of recurring orders for a specific wallet address.
6. [**Best Practices**](/recurring-api/best-practices): Best practices for using Recurring API.
