...
<Info>
  ...
  * Once low on funds, the order will not be closed and can continue to execute if the user deposits more into the order. Refer to the [Deposit Price Order](/recurring-api/deposit-price-order) endpoint to deposit more funds into the order.
  * Alternatively, the user can also withdraw funds from the order without closing it. Refer to the [Withdraw Price Order](/recurring-api/withdraw-price-order) endpoint to withdraw funds from the order.
