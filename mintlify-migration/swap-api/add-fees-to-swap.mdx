...
<Accordion title="Via Referral Program (Required only for Trigger API)">
  ...
  **1. Set up**
  ...
  You will need to complete the prerequisites and understanding of [Environment Setup](/environment-setup) and [Get Quote and Swap](/swap-api/get-quote) guide as this is reliant on the Swap API.
...
```
  ...
  Using the above, we will now know the `feeAccount` to be passed in as the parameter in Swap API. You can refer to the [Build Swap Transaction](/swap-api/build-swap-transaction) guide to add any parameters where necessary to help transaction sending, etc.
...
Finally, the user can sign the transaction and it can be submitted to the network to be executed. You can refer to the [Send Swap Transaction](/swap-api/send-swap-transaction) guide to complete this step.
