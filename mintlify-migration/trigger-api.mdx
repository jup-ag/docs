...
description: "The Jupiter Trigger API enables you to create limit orders on Solana, allowing users to set target prices for token swaps that execute automatically when market conditions are met."
---
...
1. [**Create Order**](/trigger-api/create-order): Create a new trigger order with your desired parameters.
2. [**Execute Order**](/trigger-api/execute-order): Execute a trigger order.
3. [**Cancel Order**](/trigger-api/cancel-order): Cancel an existing trigger order.
4. [**Get Trigger Orders**](/trigger-api/get-trigger-orders): Retrieve active/historical trigger orders for a specific wallet address
5. [**Best Practices**](/trigger-api/best-practices): Best practices for using Trigger API.
