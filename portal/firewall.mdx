...
<Note>
The firewall matches on request attributes for the whole team: path, IP, country, method, headers, and API key. To limit what one specific key can do, use [API Key Permissions](/portal/api-keys#permissions) instead. A permission is tied to a single key; a firewall rule applies to all of the team's traffic.
</Note>
...
A request can match more than one rule. Every match is recorded, but one rule decides the outcome, chosen by action precedence: `Bypass` > `Deny` > `Rate limit` > `Log`. Ties break toward the more specific rule, then the older one. A `Bypass` match therefore wins over any `Deny` or `Rate limit` the request also matches, and skips the firewall rate-limit checks.
...
{/* TODO: add gif - building a rule in the condition builder with the Expression Overview updating live */}
<Frame>
  <img src="/static/images/portal-firewall-rule-builder.gif" alt="Building a firewall rule in the condition builder, with the Expression Overview preview updating as conditions and the action are set" />
</Frame>
...
For example, `Path starts with /swap` `AND` `Country is any of United States, United Kingdom` matches only swap requests from those two countries. You pick countries by name; the Expression Overview shows them by name too.

Reserved and internal headers cannot be used in a `Header` condition, and one condition group can match up to 5 header values, combined with `OR`.
