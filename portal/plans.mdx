...
You can make requests without an API key at 0.5 requests per second (30 requests per minute). Keyless access is great for agentic or test use cases, but for applications running in production for users, you should use the APIs with an API key - this unlocks higher rate limits and access to all Developer Platform features such as analytics, usage monitoring, and more.
