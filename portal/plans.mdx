...
llmsDescription: "Jupiter Developer Platform pricing tiers: Free (1 RPS, no payment), Developer ($25/mo, 10 RPS, 25M credits), Launch ($100/mo, 50 RPS, 100M credits), Pro ($500/mo, 150 RPS, 500M credits). Overage billing applies to usage beyond included credits. Per-endpoint credit costs vary from 0 (execute endpoints) to 100 (portfolio positions). Keyless access available at 0.5 RPS."
---
...
{/* Overage row hidden per DEV-556, do not expose $ per credit:
...
- **Paid tiers**: Included credits reset each billing period. Usage beyond your allowance is billed.{/* Overage rate hidden per DEV-556: $1 per 1M credits */}
- **Execute endpoints are free**: All execute calls cost 0 credits

{/* Credit-to-dollar conversion hidden per DEV-556, do not expose $ per credit:
...
<AccordionGroup>
  ...
  <Accordion title="LO & DCA">
    ...
    | `/trigger/v2/orders/dca` | 1 |
    | `/trigger/v2/orders/dca/cancel/{orderId}` | 1 |
    | `/trigger/v2/orders/dca/confirm-cancel/{orderId}` | 1 |
    | `/trigger/v2/orders/history` | 1 |
    | `/trigger/v2/orders/history/dca` | 1 |
    | `/trigger/v1/createOrder` | 1 |
    | `/trigger/v1/cancelOrders` | 1 |
    | `/trigger/v1/execute` | 0 |
  </Accordion>

  <Accordion title="Recurring (unmaintained)">
    | Endpoint | Credits |
    | :--- | :--- |
