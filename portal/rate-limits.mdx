...
<Note>
Your plan's rate limit applies **per account**, not per API key. Creating multiple API keys does not increase it. [Firewall](/portal/firewall) rules can add further limits per IP, key, or team.
</Note>
...
The `/ultra` endpoints have their own bucket at **10 RPS (100 requests / 10 seconds)** on the Free tier and all paid tiers, separate from the general limit and from the `/execute` and `/submit` buckets.
