...
Each side is a Token-2022 mint (`outcomeMint`) worth \$1 if it wins and \$0 if it loses. A round only accepts orders between its `openTime` and `closeTime`; outside that window, order builds return `no_route`.
...
Both paths trade the same tokens: a Prediction API order executes as an atomic USDC-to-outcome-token swap, which is why the `outcomeMint` is also tradable through the Swap API. Deposits are **USDC only**, minimum **5 USDC**; other mints or smaller amounts are rejected at build time.
...
{/* TODO(team): the integration gist states a 100 USDC maximum, but during a live window POST /orders built successfully for 100, 150, and 200 USDC with no max error. Confirm whether a maximum is enforced (and where) before documenting a hard cap. */}

### Sell

`DELETE` returns a sell transaction in the same shape, so it reuses the helper. You can sell only while the market is tradable; after it resolves, winning tokens settle automatically (see [Claim Payouts](/prediction/claim-payouts)).
...
<CardGroup cols={2}>
  ...
  <Card title="Claim Payouts" href="/prediction/claim-payouts" icon="hand-holding-dollar">
    Settle and claim winnings after a market resolves.
  </Card>
</CardGroup>
