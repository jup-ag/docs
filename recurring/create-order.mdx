---
title: "Create Order (Recurring V1)"
sidebarTitle: "Create Order"
description: "Create a recurring DCA order with configurable frequency, amount, and start time."
llmsDescription: "UNMAINTAINED: POST /recurring/v1/createOrder sets up a DCA order with configurable cycle frequency, amount per cycle, and optional start time."
---
...
<Info>
  ...
  * Once low on funds, the order will not be closed and can continue to execute if the user deposits more into the order. Refer to the [Deposit Price Order](/recurring/deposit-price-order) endpoint to deposit more funds into the order.
  * Alternatively, the user can also withdraw funds from the order without closing it. Refer to the [Withdraw Price Order](/recurring/withdraw-price-order) endpoint to withdraw funds from the order.
...
<CardGroup>
  <Card title="Let's execute the order" href="/api-reference/recurring/execute" icon="arrow-right" horizontal/>
</CardGroup>
