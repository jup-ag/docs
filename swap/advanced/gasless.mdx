...
When a JupiterZ (RFQ) market maker wins the quote on `/order`, the market maker pays network and priority fees directly. If the taker's output token account does not exist, Jupiter's gas wallet (`gasTzr94Pmp4Gf8vknQnqxeYxdgwFjbgdJa4msYRpnB`) pays the rent to create it.
...
**What it covers:** signature fee, priority fee, and the taker's output token account rent (if the account does not exist yet).
...
- If the taker's output token account is not yet initialised, Jupiter's gas wallet (`gasTzr94Pmp4Gf8vknQnqxeYxdgwFjbgdJa4msYRpnB`) pays the rent to create it
