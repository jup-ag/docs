...
llmsDescription: "How gasless swaps work on Jupiter Swap API V2. The gasless: true response field on /order can be set by three independent paths: automatic Jupiter sponsorship (signatureFeePayer = gasTzr94Pmp4Gf8vknQnqxeYxdgwFjbgdJa4msYRpnB), a JupiterZ RFQ market maker (signatureFeePayer = MM address, with the taker still paying ATA rent), or integrator-sponsored gas via the payer parameter. Covers the three sources, how to detect which one fired via signatureFeePayer, how to opt out using excludeRouters=jupiterz or a signatureFeePayer == taker check, integrator payer setup for /order and /build, and the related fee semantics."
---
...
**What it covers:** signature fee and priority fee. ATA rent (`rentFeePayer`) remains the taker.
