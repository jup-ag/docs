...
description: "Use RTSE for optimal slippage on Meta-Aggregator and Router"
llmsDescription: "How to use Jupiter's Real-Time Slippage Estimator (RTSE) on both the Meta-Aggregator (/order) and Router (/build) paths. On the Meta-Aggregator, RTSE is applied automatically at order time. On the Router, pass slippageBps=rtse to opt in. RTSE uses heuristics (token categories, historical and real-time slippage data), algorithms (EMA), and real-time failure rate monitoring to estimate optimal slippage. Covers when to use RTSE vs a fixed slippageBps value."
---
...
If you pass `slippageBps` to `/order`, it overrides RTSE with a fixed value. This also restricts routing to Metis-only (see the [routing impact matrix](/swap/order-and-execute#routing-impact-matrix)).
...
- [Routing impact matrix](/swap/order-and-execute#routing-impact-matrix): how parameters like `slippageBps` affect which routers are available
