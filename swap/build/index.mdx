...
Sign and submit as soon as the transaction is built. The longer you wait, the more the quote goes stale as onchain prices move. The `blockhashWithMetadata.lastValidBlockHeight` field is the hard expiry, but price movement can cause slippage failures well before then.

The `blockhashSlotsToExpiry` parameter (default 150 slots, roughly 60-90 seconds) controls how far into the future the blockhash is valid. This is a ceiling, not a target. Sign and submit immediately.

Send via your own RPC, or use [`/submit`](/transaction/submit) to land through Jupiter's transaction infrastructure with SOL tips.
