...
Validate the compute unit price from the response before signing. The estimate tracks recent network priority fees and can spike when recent blocks contain transactions bidding extreme values. See [Cap the compute unit price](/swap/advanced/compute-units#cap-the-compute-unit-price) for a decode-and-clamp snippet.
...
| `dexes` | (all) | Restrict routing to specific DEXes. Labels are case sensitive. See the [full DEX label list](/api-reference/swap/program-id-to-label) |
| `excludeDexes` | (none) | Exclude specific DEXes from routing. Labels are case sensitive. See the [full DEX label list](/api-reference/swap/program-id-to-label) |
...
| `forJitoBundle` | false | Exclude DEXes incompatible with Jito bundles |
