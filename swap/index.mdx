...
| **Transaction landing** | Managed via `/execute` (optimised slippage, priority fees, our proprietary landing pipeline) | Self-managed via your own RPC, or via [`/submit`](/swap/submit) with SOL tips for Jupiter Beam landing (our proprietary landing pipeline) |
...
| [`/submit`](/swap/submit) | POST | Submit any signed transaction through Jupiter's landing infrastructure with SOL tips |

**Base URL:** `https://api.jup.ag/swap/v2` (except `/submit` which is at `https://api.jup.ag/submit`)
