...
| **JupiterZ** | RFQ (Request for Quote) system where market makers compete to provide off-chain liquidity. Often beats onchain by 5-20bps on major pairs. | Meta-Aggregator + [standalone JupiterZ API](/swap/jupiterz) |
| **Dflow** | Third-party on-chain router. | Meta-Aggregator only |
| **OKX** | Third-party OKX liquidity. | Meta-Aggregator only |

<Note>
JupiterZ (RFQ) is not available on the Router path. It is available through the Meta-Aggregator, where it competes with the other engines, and through the [standalone JupiterZ API](/swap/jupiterz), which quotes RFQ market makers only.
...
| POST | [`tx.jup.ag`](/transaction/submit) | Land any signed transaction through Jupiter's infrastructure via Solana `sendTransaction` with SOL tips |

**JupiterZ (RFQ)**

| Method | Endpoint | Description |
| --- | --- | --- |
| GET | [`/swap/v2/jupiterz/order`](/swap/jupiterz) | Get a firm RFQ quote and unsigned transaction |
| GET | [`/swap/v2/jupiterz/global-order`](/swap/jupiterz#global-order) | Best RFQ quote across multiple candidate output mints |
| POST | [`/swap/v2/jupiterz/execute`](/swap/jupiterz#executing) | Submit the taker-signed transaction for the market maker to fill |
...
- **AMM operators**: [Integrate AMM into Metis](/swap/routing/amm/integration) to get your markets included in Jupiter's onchain routing.
- **Market makers**: integrate into JupiterZ to provide off-chain liquidity via the RFQ system, over a [webhook (V1)](/swap/routing/rfq/v1/overview) or [gRPC streams (V2)](/swap/routing/rfq/v2/streaming).
