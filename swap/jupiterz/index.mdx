...
This page is for integrators consuming quotes: wallets, aggregators, trading UIs and bots. On the [Meta-Aggregator path](/swap/order-and-execute), JupiterZ competes with the other routers for the best price; the JupiterZ API skips the competition and quotes RFQ market makers only. If you are a market maker looking to provide liquidity, see [Integrate MM into JupiterZ](/swap/routing/rfq/v1/overview).
...
<Note>
Access to the JupiterZ API is whitelisted per organization, not per key: your Developer Platform organization must be enabled for it, after which any API key belonging to that organization works. To request access, reach out via the [enterprise form](https://airtable.com/apppC9l7PaT9XG8jN/pagjXGV6PFNwUd9P2/form).
</Note>
...
`expireAt` is a Unix timestamp (seconds) after which the transaction is no longer valid. You cannot execute right up to it: `/execute` needs a safety buffer of at least 20 seconds before `expireAt` to verify, sign, and land the fill, so it rejects once fewer than 20 seconds remain. In practice a 55-second quote is only executable for roughly its first 35 seconds. Do not build a user countdown off `expireAt`, subtract the buffer. Executing an expired quote returns `400` with `errorCode: "QUOTE_EXPIRED"`; request a fresh order instead.
...
| `error` | Meaning |
| :--- | :--- |
| `insufficientBalance` | The taker cannot cover the swap. This is not only the input token: the taker also pays rent for any token account the swap has to create, so a taker holding plenty of the input token but too little SOL for that rent also returns `insufficientBalance` |
...
Errors raised while processing a request use this JSON body:
...
Two things to know about the `error` field:

- On a `400` from bad or missing parameters, `error` is a **request UUID**, not a readable message. The human-readable reason is written to Jupiter's logs, so quote this UUID when asking support.
- Malformed query strings are rejected before this JSON body is produced. A missing query parameter returns **plain text**, not JSON, for example `Failed to deserialize query string: missing field \`amount\``. Only errors raised on the request body use the JSON shape above.
...
- [Integrate MM into JupiterZ](/swap/routing/rfq/v1/overview): provide liquidity as a market maker
