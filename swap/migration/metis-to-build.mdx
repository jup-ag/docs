---
title: "Metis to Router"
description: "Migrate from Metis /quote + /swap-instructions to /build"
llmsDescription: "Migration guide for moving from Jupiter Metis Swap API V1 (/quote + /swap-instructions) to the Swap API V2 Router path (/build). Covers parameter mapping from /quote to /build, response mapping from /swap-instructions to /build, V1 to V2 instruction differences (no fee events, percent to bps change in routePlan, ALT format change), complete before-and-after TypeScript code examples, and what you gain from migrating."
---
...
**V1 `/swap` returned an assembled base64-encoded transaction.** V2 `/build` returns individual instruction objects (`computeBudgetInstructions`, `setupInstructions`, `swapInstruction`, etc.) that you assemble into a transaction yourself. This gives you full control to insert custom instructions, but means you are responsible for building the v0 transaction, simulating compute units, and signing. See the [Build](/swap/build) page for the complete workflow.
...
- [Build](/swap/build) for the full `/build` workflow and code examples
- [Common Instructions](/swap/build/common-instructions) for composing custom instructions with your swap
