...
The Meta-Aggregator is the recommended way to swap on Jupiter. All routing engines compete for the best price (Metis, JupiterZ RFQ, Dflow, OKX), and Jupiter handles transaction landing for you via `/order` + `/execute`. To source RFQ liquidity only, without the router competition, use the [standalone JupiterZ API](/swap/jupiterz).
