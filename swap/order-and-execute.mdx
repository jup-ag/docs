...
- Optimised priority fee strategy for current network conditions. To bound the priority fee yourself, set `priorityFeeLamports` on `/order` to a fixed value. This overrides Jupiter's estimate and counts as a custom parameter (see [Order mode](#order-mode))
