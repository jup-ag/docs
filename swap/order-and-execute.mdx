...
| `errorCode` | Present when `transaction` is `""`. Numeric code identifying the error. |
| `errorMessage` | Present when `transaction` is `""`. Human-readable description (do not match on this string, it may change). |
...
```typescript
if (!order.transaction) {
  // order.errorCode and order.errorMessage explain why
  console.error(`Order has no transaction: [${order.errorCode}] ${order.errorMessage}`);
