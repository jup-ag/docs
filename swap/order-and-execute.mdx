...
| `errorCode` | Present when `transaction` is `""`. Numeric code identifying the error. Match on `router` + `errorCode` (see tables below). |
| `errorMessage` | Present when `transaction` is `""`. Human-readable description. Do not match on this string, it may be parameterised. |

When `transaction` is an empty string, the response still contains valid pricing (`inAmount`, `outAmount`) but the swap cannot proceed. The same `errorCode` number has different meanings depending on the `router`. Match on both fields:

**Aggregator routers** (`iris`, `dflow`, `okx`, `mantis`):
...
**Key takeaway:** adding `referralAccount`, `referralFee` or `payer` disables JupiterZ (RFQ), which may result in worse pricing on major pairs where market makers often beat onchain routing by 5-20bps.
