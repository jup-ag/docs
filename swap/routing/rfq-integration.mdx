...
llmsDescription: "How to integrate a market maker into Jupiter's JupiterZ RFQ (Request for Quote) system. Market makers host a webhook implementing the RFQ API schema, pass fulfillment rate and response time requirements, and onboard into JupiterZ. RFQ liquidity is available on the /order path only (not /build). Covers webhook requirements, the quoting and settlement flow, fulfillment expectations, and how to contact Jupiter for onboarding."
---
...
<Note>
    For latest info on JupiterZ integration, please refer to https://jupiterz.jup.ag/
</Note>
