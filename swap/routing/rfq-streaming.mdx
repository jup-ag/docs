...
<Note>
V2 runs concurrently with the [V1 webhook model](/swap/routing/rfq-integration); it does not replace it. You can integrate with either or both. Both settle through on-chain order-engine programs.
</Note>
...
On-chain, fills settle through the `rfq_v2` program ([`fd3nMFYTQjX1yr5ER8u7tPdHJB7qt8RpDpNtLQX2Br5`](https://solscan.io/account/fd3nMFYTQjX1yr5ER8u7tPdHJB7qt8RpDpNtLQX2Br5)) with the single instruction `fill_exact_in`. The program reads the instructions sysvar and rejects the fill with `MakerAppearsInOtherInstruction` (error `6005`) if your keys appear in any other instruction, defeating wrap-and-drain attacks even if your off-chain check misses one.
