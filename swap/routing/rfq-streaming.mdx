...
<Note>
V2 runs concurrently with the [V1 webhook model](/swap/routing/rfq-integration); it does not replace it. You can integrate with either or both. The two models sit in different parts of Jupiter's stack: V1 works directly in the JupiterZ program and APIs, while V2 liquidity is consumed by Metis and fills execute through the Jupiter v6 swap aggregator program.
</Note>
...
| Settlement | Directly through the JupiterZ [Order Engine program](/swap/routing/rfq-integration#order-engine) in a dedicated RFQ transaction | Through the Jupiter v6 swap aggregator program (`JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4`) as part of a Metis route, which invokes the `rfq_v2` fill program |
...
- [Webhook Integration (V1)](/swap/routing/rfq-integration): the request-response integration model
