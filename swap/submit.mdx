...
<Note>
This endpoint is currently shared privately. It is not a public-facing endpoint.
</Note>
...
The example code builds on the [`/build` endpoint](/swap/build).

Call `/build` with `tipAmount`, build and sign the transaction, then submit via `/submit`.
