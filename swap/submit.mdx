...
description: "Submit signed transactions through Jupiter's infrastructure with SOL tips"
llmsDescription: "How to submit signed transactions through Jupiter's transaction landing infrastructure using POST /swap/v2/submit. Accepts any signed Solana transaction with a SOL tip (minimum 0.001 SOL) to one of 16 Jupiter V6 program authorities. Designed for /build transactions and non-Jupiter transactions that want access to Jupiter Beam's landing pipeline. Differs from /execute which is tied to the /order flow and uses swap fees. Includes TypeScript code examples for both @solana/kit and @solana/web3.js, standalone tip instruction examples for non-Jupiter transactions, and the full list of tip recipient addresses."
hidden: true
---

`/submit` lets you send signed transactions through Jupiter's transaction landing infrastructure with SOL tips for priority processing. It runs on the same pipeline as Jupiter Beam, giving you performant transaction landing for any transaction type.

<Note>
This endpoint is currently shared privately with close partners. It is not a public-facing endpoint.
</Note>
...
| **Routing** | All routers (Metis + RFQ) | N/A (you build the transaction yourself) |
| **Use case** | Recommended swap happy path | Custom transactions, `/build` transactions, non-Jupiter swaps |

When `/submit` goes public, the two may converge into a unified submission flow since they share the same underlying infrastructure.
...
| Transaction types | Any signed Solana transaction (private distribution)                |
