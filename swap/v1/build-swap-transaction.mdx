...
description: "Build a serialized swap transaction from a Metis quote with configurable compute and fee settings."
llmsDescription: "UNMAINTAINED: POST /swap/v1/swap builds a serialized swap transaction from a quote. Supports dynamicComputeUnitLimit, dynamicSlippage, and prioritizationFeeLamports. Also covers /swap-instructions for instruction-level control and CPI/Flash Fill integration."
---
...
In this guide, we will pick up from where [**Get Quote**](/swap/get-quote) guide has left off.
...
<Tip>
  **OPTIMIZING FOR TRANSACTION LANDING IS SUPER SUPER IMPORTANT!**
  ...
  To understand how these parameters help, the next step, [Send Swap Transaction guide](/swap/send-swap-transaction) will discuss them.
</Tip>
...
<CardGroup>
  <Card title="Let’s go sign and send!" href="/swap-api/send-swap-transaction" icon="paper-plane" horizontal/>
</CardGroup>
...
<Info>
  **NOTE**
  ...
  [Refer to the GET /quote's `maxAccounts` guide for more details.](/swap/get-quote#max-accounts)
</Info>
