...
description: "Sign and send a Metis swap transaction with guidance on priority fees, compute units, and broadcasting."
llmsDescription: "UNMAINTAINED: Sign and send the serialized Metis swap transaction via your own RPC. Covers priority fee estimation with prioritizationFeeLamports, compute unit optimisation with dynamicComputeUnitLimit, dynamic slippage, and Jito tip broadcasting."
---
...
In this guide, we will pick up from where [**Get Quote**](/swap/get-quote) and [**Build Swap Transaction**](/swap/build-swap-transaction) guide has left off.

If you have not set up your environment to use the necessary libraries, the RPC connection to the network and successfully get a quote from the Quote API, please start at [Environment Setup](/get-started/environment-setup) or [get quote](/swap/get-quote).
...
You can pass in `dynamicComputeUnitLimit` to the Metis Swap API, which simulates the swap to provide an accurate compute unit limit in the transaction.

When `true`, it ensures the transaction uses the correct compute unit limit rather than a default value. This is recommended for all normal flows. An accurate limit reduces the priority fees needed and improves the chance of your transaction landing in a block.
...
<Tip>
  ...
  - [Alternatively, consider using the Ultra Swap API](/ultra):
...
<Note>
  ...
  * **Monitoring**: Real-time monitoring of failure rates to ensure reactiveness to increase slippage when necessary.
  ...
  <Card title="Refer to Ultra Swap API for more information on RTSE" href="/ultra#real-time-slippage-estimator" icon="book-open" horizontal/>
</Note>
