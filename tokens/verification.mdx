...
llmsDescription: "Jupiter VRFD Express Verification API for programmatic token verification and metadata updates. Three-step flow: check eligibility (GET /tokens/v2/verify/express/check-eligibility), craft a payment transaction worth 1000 JUP (GET /tokens/v2/verify/express/craft-txn), then execute with the signed transaction and token details (POST /tokens/v2/verify/express/execute). Payment can be made in JUP, SOL, USDC, or JUPUSD; non-JUP currencies are swapped to 1000 JUP via Ultra. Returns swap sizing fields (quotedInputAmount, maxInputAmount, inputMint) on non-JUP paths and reports verificationCreated and metadataCreated on execute. Used by launchpads, agents, and projects to submit tokens for Jupiter verification without the UI. Requires an API key from developers.jup.ag/portal. Verification and metadata updates are reviewed independently by the VRFD team."
---
...
Each submission costs 1000 JUP. You can pay in JUP directly, or in SOL, USDC, or JUPUSD, which are swapped to 1000 JUP via Ultra. All paths settle 1000 JUP to a Jupiter treasury wallet, and the JUP is burned periodically. There is also a free standard submission flow on the [VRFD site](https://verified.jup.ag).
...
`paymentCurrency` takes one of the token symbols below, passed verbatim as a string. Use the symbol (for example `SOL`), not the mint address or token name. The mint column shows what each symbol resolves to on-chain. JUP is the default and transfers directly. SOL, USDC, and JUPUSD are routed through an Ultra swap so that 1000 JUP still settles to the treasury.

| `paymentCurrency` value | Resolves to mint | Decimals | How it settles |
| --- | --- | --- | --- |
...
2. The submitting wallet needs enough of your chosen payment currency to cover 1000 JUP. For JUP that is 1000 JUP. For SOL, USDC, or JUPUSD it is the swap input amount returned by craft-txn. Transactions are gasless, so the wallet does not need extra SOL for network fees.
...
| `mint` | `string` | Mint of the settled payment token (JUP) |
| `amount` | `string` | JUP amount in smallest unit (1000 JUP = `1000000000`) |
| `tokenDecimals` | `number` | Decimals of the settled token (JUP is 6) |
| `tokenUsdRate` | `number` | USD rate of the settled token at craft time |
