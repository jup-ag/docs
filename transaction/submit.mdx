...
| `skipPreflight` | Must be `true`. An explicit `false` is rejected; omit to default to `true` |
| `maxRetries` | Must be `0`. A non-zero value is rejected; omit to default to `0` |
| `swqosOnly` | Optional. `true` routes only through stake-weighted QoS for lowest latency, no Jito tip needed. Set it in the raw JSON-RPC config, as SDK send helpers may not forward it |
