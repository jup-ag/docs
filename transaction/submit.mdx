...
| **API access** | Pass your Jupiter API key in the `x-api-key` header. Keyless access (no header) works at a lower rate limit |

<Tip>
The API-gateway submit path `POST api.jup.ag/tx/v1/submit` has a dedicated rate limit bucket (Keyless 20, Free 50, Paid 100 RPS), separate from your general API limit. See [Rate Limits](/portal/rate-limits#buckets). `tx.jup.ag` routes directly to Beam rather than through the API gateway, so its rate limits are applied separately.
</Tip>
...
- **Set `skipPreflight: true` and `maxRetries: 0`:** `tx.jup.ag` sends directly to the TPU and does not simulate, so preflight adds nothing. Let your application own retry logic so you can refresh the blockhash between attempts.
