...
<Note>
If you omit `authMode`, the response is the deprecated `access_only` shape: `{ "authMode": "access_only", "token": "<jwt, expires in 24 hours>" }`, returned with `Deprecation` and `Link` headers. Read `accessToken`, not `token`, once you migrate.
</Note>
