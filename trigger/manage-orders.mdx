...
Update an order in place, or cancel it with a two-step withdrawal. Every request needs the JWT `token` from [authentication](/trigger/authentication) and the `orderId` returned when you [created the order](/trigger/create-order) (the same value as `id` in [order history](/trigger/order-history)).
