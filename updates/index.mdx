...
* We've added a new parameter to the Legacy Swap API that lets you send native SOL 
...
* The `routePlan[].percent` field in the Legacy Swap API quote response may return `null` - due to an an upgrade on the routing algorithm.
...
### Deprecate `ctLikes` and `smartCtLikes` response field in search endpoints
* We'll be deprecating the `ctLikes` and `smartCtLikes` response field in the search endpoints.
* This will take effect next week and it affects both search endpoints in Ultra API and Tokens API.
* If you are using the field for any purpose, please move off and stop using it.

### Remove `feeAmount` and `feeMint` fields in `SwapInfo`
* We will be removing the `feeAmount` and `feeMint` fields from the `routePlan[].swapInfo` schema in a quote response.
