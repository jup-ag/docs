...
* **`GET /order`** — default path. Returns an assembled transaction with all Ultra benefits (RTSE, Jupiter Beam, gasless, multi-router competition including RFQ). Sign and send to `/execute`.
* **`GET /build`** — advanced path. Returns raw swap instructions for building custom transactions (CPI, custom instructions, composability). No Jupiter swap fees.
* **`POST /execute`** — managed execution for `/order` transactions.

Ultra and Metis APIs remain available but they are no longer being maintained and will be fully deprecated in the future. See the [migration guide](/docs/swap/v2/migration) for moving from Metis or Ultra to the new API.

- [Swap API V2 overview](/docs/swap)
...
Comprehensive documentation for Jupiter Lend (powered by Instadapp) is now available, covering the full Lend product suite.
