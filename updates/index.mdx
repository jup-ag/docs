...
## Jupiter Lend API & SDK

Comprehensive documentation for Jupiter Lend (powered by Instadapp) is now available, covering the Lend REST API and SDK.

### Lend Earn API

The Earn API at `api.jup.ag/lend/v1/earn/` now has full documentation, including:

* `POST /deposit` and `POST /withdraw`: deposit/withdraw by asset amount
* `POST /mint` and `POST /redeem`: deposit/withdraw by share count
* `GET /tokens`: list all supported vaults with supply rates and liquidity
* `GET /positions`: query user position data (shares, underlying assets, balance)
* `GET /earnings`: get earnings for specific positions
* All endpoints also have `-instructions` variants for CPI and composability

### Lend SDK

* **Earn SDK:** `getDepositIxs`, `getWithdrawIxs`, read-data helpers for jlToken shares and balances
* **Borrow SDK:** Create positions, deposit collateral, borrow, repay, withdraw, and combined single-transaction operations
* **Advanced Recipes:** Multiply, Unwind, Repay-with-Collateral, and Vault Swap using CPI + Jupiter swap
* **Flash Loans:** Borrow and repay within the same transaction
